...
import com.tugalsan.java.lib.vnc.desktop.server.rfb.*;
import java.util.*;
...
public class TS_LibVncDesktopViewerSwing_GuiOptionsDialog extends JDialog {

    private JSlider jpegQuality;
    private JSlider compressionLevel;
    private JCheckBox viewOnlyCheckBox;
    private TS_LibVncDesktopRfbProtocol_Settings settings;
    private JCheckBox sharedSession;

    private RadioButtonSelectedState<TS_LibVncDesktopRfbProtocol_LocalPointer> mouseCursorTrackSelected;
    private Map<TS_LibVncDesktopRfbProtocol_LocalPointer, JRadioButton> mouseCursorTrackMap;
    ...
    public TS_LibVncDesktopViewerSwing_GuiOptionsDialog(Window owner) {
    ...
    public void initControlsFromSettings(TS_LibVncDesktopRfbProtocol_Settings settings, TS_LibVncDesktopViewer_SettingsUi uiSettings, boolean isOnConnect) {
    ...
    private JPanel createEncodingsPanel() {
        ...
        encodings.addItem(new EncodingSelectItem(TS_LibVncDesktopRfbEncoding_Type.TIGHT));
        encodings.addItem(new EncodingSelectItem(TS_LibVncDesktopRfbEncoding_Type.HEXTILE));

//		encodings.addItem(new EncodingSelectItem(EncodingType.RRE));
//		encodings.addItem(new EncodingSelectItem(EncodingType.ZLIB));
        encodings.addItem(new EncodingSelectItem(TS_LibVncDesktopRfbEncoding_Type.ZRLE));
        encodings.addItem(new EncodingSelectItem(TS_LibVncDesktopRfbEncoding_Type.RAW_ENCODING));
    ...
    private static class EncodingSelectItem {
        ...
        final TS_LibVncDesktopRfbEncoding_Type type;

        EncodingSelectItem(TS_LibVncDesktopRfbEncoding_Type type) {
            this.type = type;
        }
    ...
    private JPanel createColorDepthPanel() {
        ...
        colorDepth.addItem(new ColorDepthSelectItem(TS_LibVncDesktopRfbProtocol_Settings.COLOR_DEPTH_SERVER_SETTINGS,
                "Server's default"));
        colorDepth.addItem(new ColorDepthSelectItem(TS_LibVncDesktopRfbProtocol_Settings.COLOR_DEPTH_24,
                "16 777 216 colors"));
        colorDepth.addItem(new ColorDepthSelectItem(TS_LibVncDesktopRfbProtocol_Settings.COLOR_DEPTH_16,
                "65 536 colors"));
        colorDepth.addItem(new ColorDepthSelectItem(TS_LibVncDesktopRfbProtocol_Settings.COLOR_DEPTH_8,
                "256 colors"));
        colorDepth.addItem(new ColorDepthSelectItem(TS_LibVncDesktopRfbProtocol_Settings.COLOR_DEPTH_6,
                "64 colors"));
        colorDepth.addItem(new ColorDepthSelectItem(TS_LibVncDesktopRfbProtocol_Settings.COLOR_DEPTH_3,
                "8 colors"));
    ...
    private boolean whetherJpegQualityPaneBeEnabled(int depth) {
        return TS_LibVncDesktopRfbProtocol_Settings.COLOR_DEPTH_16 == depth
                || TS_LibVncDesktopRfbProtocol_Settings.COLOR_DEPTH_24 == depth
                || TS_LibVncDesktopRfbProtocol_Settings.COLOR_DEPTH_SERVER_SETTINGS == depth;
    }
...
//                mouseCursorShapeTrackGroup);
    ...
    private JPanel createMouseCursorPanel() {
        ...
        addRadioButton("Track remote cursor locally", TS_LibVncDesktopRfbProtocol_LocalPointer.ON,
        ...
        addRadioButton("Let remote server deal with mouse cursor",
                TS_LibVncDesktopRfbProtocol_LocalPointer.OFF,
                mouseCursorTrackSelected, mouseCursorTrackMap, mouseCursorBox,
                mouseCursorTrackGroup);
        addRadioButton("Don't show remote cursor", TS_LibVncDesktopRfbProtocol_LocalPointer.HIDE,
                mouseCursorTrackSelected, mouseCursorTrackMap, mouseCursorBox,
                mouseCursorTrackGroup);
