...
public class TS_LibVncDesktopViewerSwing_RendererImpl extends TS_LibVncDesktopDrawing_Renderer implements ImageObserver {
    ...
    public TS_LibVncDesktopViewerSwing_RendererImpl(TS_LibVncDesktopTransport_Transport transport, int width, int height, TS_LibVncDesktopRfbEncoding_PixelFormat pixelFormat) {
        if (0 == width) {
            width = 1;
        }
        if (0 == height) {
        ...
        cursor = new TS_LibVncDesktopViewerSwing_SoftCursorImpl(0, 0, 0, 0);
    }
    ...
    public void drawJpegImage(byte[] bytes, int offset, int jpegBufferLength,
            TS_LibVncDesktopRfbEncoding_FramebufferUpdateRectangle rect) {
    ...
    public void paintCursorOn(Graphics g, boolean force) {
        synchronized (cursor.getLock()) {
            var cursorImage = ((TS_LibVncDesktopViewerSwing_SoftCursorImpl) cursor).getImage();
    ...
    public TS_LibVncDesktopViewerSwing_SoftCursorImpl getCursor() {
        return (TS_LibVncDesktopViewerSwing_SoftCursorImpl) cursor;
    }
